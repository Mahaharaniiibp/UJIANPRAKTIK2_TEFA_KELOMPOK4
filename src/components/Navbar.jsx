...
const Navbar = () => {
  ...
  return (
    <nav className="fixed w-full left-0 top-0 z-[999] bg-yellow-300 text-gray-900">
      <div className="flex items-center justify-between">
        ...
        <div className="md:bg-white/0 bg-white text-gray-900 md:block hidden px-7 py-2 font-medium bg-white rounded-bl-full">
          ...
          <ul className="flex flex-col justify-center h-full gap-10 py-2 text-lg">
            {menuLinks?.map((menu, i) => (
              <li
                ...
                key={i}
                className="px-6 hover:text-orange-00"
              >
                <a href={menu?.link}>{menu?.name}</a>
              </li>
            ))}
